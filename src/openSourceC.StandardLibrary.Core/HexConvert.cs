...
namespace openSourceC.StandardLibrary
{
	...
	public static class HexConvert
	{
		...
		public static string ByteArrayToString(byte[] byteArray, int offset, int length)
		{
			...
			if (offset < 0 || offset >= byteArray.Length) { throw new ArgumentOutOfRangeException(nameof(offset)); }
			if (length < 0 || offset + length > byteArray.Length) { throw new ArgumentOutOfRangeException(nameof(length)); }
