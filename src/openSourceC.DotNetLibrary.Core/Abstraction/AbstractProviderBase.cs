...
using SR = openSourceC.DotNetLibrary.SR;
		...
		where TProviderSettings : ProviderSettings, new()
	{
			...
			where TInterface : class
		{
			try
			{
				...
				{
					throw new NullValueException($"{nameof(settings.Type)} is null.");
				}
...
#if true
				Type providerType = Type.GetType(settings.Type) ?? throw new NullValueException($"Unable to create type ${settings.Type}.");
				...
				object instance = Activator.CreateInstance(
					...
					null,
					null
				) ?? throw new NullValueException($"Unable to create instance of ${providerType.AssemblyQualifiedName}.");
#else
